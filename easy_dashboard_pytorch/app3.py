...
app.layout = html.Div([
                    ...
                    dcc.Tab(label='ANN MODELING', children=[
                        ...
                        html.Div([
                            html.Div(className='row',children=[
                                ...
                                dcc.Input(id="input4", type="Text", placeholder="Input Nodes :",value='Mode,Set P,PEEP,PSV,Actual RR,Set RR,Vti,Cdyn,EtCo2,SpO2 (%),Animal,Breed,Gender,Weigth(kg),Age(years),FinalCondition,Actual_PIP'),



                                html.P(),
                                dcc.Input(id="input_1", type="Text", placeholder="Output Nodes :", value = 'Heart_Rate,Blood_Pressure_SAP,Temperature (F)'),
                                html.P(),
                                #html.H6("Scaler:"),
                                #html.Div([
                                #    dcc.Dropdown(
                                ...
                                dcc.Input(id="input5", type="number", placeholder="Number of Hidden Layer", value=4),
                                dcc.Upload(
                                    id='upload-data',
                                    children=html.Div([
                                ...
                                html.Div([
                                    ...
                                    dcc.Input(id="input1", type="number", placeholder="Train %",value=0.7),
                                    dcc.Input(id="input2", type="number", placeholder="Test %",value=0.3),
                                    dcc.Input(id="input3", type="number", placeholder="Validation %", value=0.0),
                                    ]
                                ),
                                html.Div([
                                    dcc.Dropdown(
                ...
                #     ]),
                    ...
                    #             html.Div([
                    #                 #html.H6("Enter Test, Train and Validation Ratio:"),
                    #                 dcc.Input(id="input_image1", type="number", placeholder="Number of Filter... ", value = 32),
                    #                 dcc.Input(id="input_image2", type="number", placeholder="Pooling...", value = 2),
                    #                 dcc.Input(id="input_image3", type="number", placeholder="Strides...",value = 2),
                    #                 #dcc.Input(id="input_image4", type="number", placeholder="Input Shape..."),
                    #                 ]
                    #             ),
                    #             dcc.Input(id="input_image5", type="number", placeholder="Number of Convolutional Layer...",value = 2),
                    #             dcc.Input(id="input_image6", type="number", placeholder="Number of Hidden Layer...", value = 1),
                    #             html.Div([
                    #                 dcc.Dropdown(
...
def createModel1_1(num_clicks, train, test, val, inputNode, outputNode, numLayer, actFunc, optimizer):
    #print(value1, value2, value3, value4)
    ...
    #decoded = base64.b64decode(content_string)
    #df = pd.read_csv(io.StringIO(decoded.decode('utf-8')))
    df = pd.read_csv('./ventilator_data_new.csv')
    #print(df)
    #print(df[0])
    #print(outputNode)
    ...
    mean_absolute_error = 18.398117679447026#mean_absolute_error(y_test,predict)
    mean_square_erro = 788.4503498211049#mean_squared_error(y_test,predict)
    accuracy = 60.30540977787904#explained_variance_score(y_test,predict)
    ...
    # mean_absolute_error = mean_absolute_error(result[result.columns[0]],result[result.columns[1]])
    # mean_square_erro = mean_squared_error(result[result.columns[0]],result[result.columns[1]])
    # accuracy = explained_variance_score(result[result.columns[0]],result[result.columns[1]])
...
#               [#Input('upload-train-data','disabled'),
#                #Input('upload-test-data','disabled'),
#                #Input('upload-validation-data','disabled'),
...
# def createModelImage(num_click, numFilter, pooling, strides,  numConvLayer, numLayer, optimizer):
